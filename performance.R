performance.calcCNRRatio <-function (gA, a, f, vis, distance, p, lambda, deltaMu, deltaPhi) {
  ...
  sampleAttenuatiuonFactor = 2*f
  giFactors = vis*distance*1e3/p
  sampleSignal = deltaPhi/(deltaMu)
  # Calc CNR ratio (phase over abs)
  nu = spatialResolutionFactor*sampleAttenuatiuonFactor*(giFactors*lambda*abs(sampleSignal))^2
  return(sqrt(nu))
}
